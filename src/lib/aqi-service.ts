 ...
 * Priority: Scraped aqi.in (exact parity) > WAQI API > CPCB > Mock Data
 */
export async function getAQIData(): Promise<AQIStation[]> {
    ...
    try {
        // Try aqi.in scraper FIRST for exact parity with aqi.in website
        console.log('Attempting to scrape from aqi.in for exact parity...');
        const scrapedData = await scrapeAQIIn();

        if (scrapedData && scrapedData.delhiAqi > 0) {
            console.log(`Scraped aqi.in: Delhi AQI = ${scrapedData.delhiAqi}, PM2.5 = ${scrapedData.pm25}, PM10 = ${scrapedData.pm10}`);
            const scrapedStations = createStationsFromScrapedData(scrapedData);

            // Supplement with mock stations for additional coverage (Delhi-NCR only)
            const stationNames = new Set(scrapedStations.map(s => s.name.toLowerCase()));

            // Filter mock stations to Delhi-NCR region only
            // Delhi-NCR bounds: lat 28.3-29.0, lng 76.8-77.6
            const isDelhiNCR = (lat: number, lng: number) =>
                lat >= 28.3 && lat <= 29.0 && lng >= 76.8 && lng <= 77.6;

            const updatedMock = mockStations
                .filter(s => !stationNames.has(s.name.toLowerCase()))
                .filter(s => isDelhiNCR(s.lat, s.lng)) // Only Delhi-NCR stations
                .map(station => {
            ...
            const combinedStations = [...scrapedStations, ...updatedMock];
            ...
            console.log(`Total stations from scraper: ${combinedStations.length} (Scraped: ${scrapedStations.length}, Mock: ${updatedMock.length})`);
            return combinedStations;
        }

        // Fallback: Try WAQI API if scraper fails
        console.log('Scraper failed, falling back to WAQI API...');
        const waqiData = await fetchWAQIData();

        if (waqiData && waqiData.aqi > 0) {
            console.log(`WAQI API fallback: Delhi AQI = ${waqiData.aqi}`);
            const waqiStations = convertWAQIToStations(waqiData);

            // Use WAQI data with Delhi-NCR mock supplement
            const isDelhiNCR = (lat: number, lng: number) =>
                lat >= 28.3 && lat <= 29.0 && lng >= 76.8 && lng <= 77.6;
            ...
            const updatedMock = mockStations
                ...
                .filter(s => isDelhiNCR(s.lat, s.lng))
                .map(station => {
                    const variance = (Math.random() - 0.5) * 80;
            ...
            console.log(`Total stations from WAQI: ${combinedStations.length}`);
            return combinedStations;
        }

        // Last resort: Use CPCB data only
        ...
        // Start with CPCB stations
        const allStations = [...cpcbStations];

        // No CPCB duplicates check needed since we start fresh
        const existingNames = new Set(cpcbStations.map(s => s.name.toLowerCase()));
        ...
        if (allStations.length > 0) {
            ...
            // Update mock stations with scraped AQI if available (Delhi-NCR only)
            // Delhi-NCR bounds: lat 28.3-29.0, lng 76.8-77.6
            const isDelhiNCR = (lat: number, lng: number) =>
                lat >= 28.3 && lat <= 29.0 && lng >= 76.8 && lng <= 77.6;
            ...
            console.log(`Total stations from CPCB fallback: ${combinedStations.length}`);
            return combinedStations;
        }
