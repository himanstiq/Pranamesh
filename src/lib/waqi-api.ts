...
// Demo token for fallback (rate-limited but works for basic usage)
...
export async function fetchWAQIData(): Promise<WAQIResponse | null> {
    ...
    // Use configured token or fallback to demo token
    const token = process.env.WAQI_API_TOKEN || DEMO_TOKEN;
    const isDemo = !process.env.WAQI_API_TOKEN;

    if (isDemo) {
        console.log('Using demo WAQI token (rate-limited)');
    }

    console.log('Fetching real-time data from WAQI API...');

    try {
        // Fetch primary Delhi station first - use 'new-delhi' which works reliably
        const primaryData = await fetchStationData('new-delhi', token);
