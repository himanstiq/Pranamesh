...
// Generate comprehensive station list from mock data
export const DELHI_STATIONS: StationInfo[] = aqiStations.map(station => ({
    id: station.id,
    name: station.name,
...
// Get stations grouped by type
export function getStationsByType() {
    return {
        government: DELHI_STATIONS.filter(s => s.type === 'government'),
        industrial: DELHI_STATIONS.filter(s => s.type === 'industrial'),
        pranamesh: DELHI_STATIONS.filter(s => s.type === 'pranamesh'),
    };
}
...
/**
 * Generate historical data for custom date range
 */
function generateCustomRangeData(currentAqi: number, startDate: string, endDate: string): HistoricalDataPoint[] {
    const data: HistoricalDataPoint[] = [];
...
export async function getHistoricalData(
    stationId: StationId = 'aicte-delhi',
    range: TimeRange = '24h',
    customRange?: CustomDateRange
): Promise<HistoricalAPIResponse> {
    const cacheKey = customRange
        ? `${stationId}-custom-${customRange.startDate}-${customRange.endDate}`
        : `${stationId}-${range}`;
    ...
    try {
        ...
        if (!waqiData) {
            ...
            const data = generateDataForRange(fallbackAqi, range, customRange);
        ...
        const data = generateDataForRange(currentAqi, range, customRange);
 ...
 */
function generateDataForRange(
    currentAqi: number,
    range: TimeRange,
    customRange?: CustomDateRange
): HistoricalDataPoint[] {
    if (range === 'custom' && customRange) {
        return generateCustomRangeData(currentAqi, customRange.startDate, customRange.endDate);
    }
